...
def getInfosThread(codeASIN, size_name, infos, indexSort):
    ...
    info = {'codeasin': codeASIN, 'sizeName': size_name,  
            'isSoldOut': isSoldOut, 'price': price, 'indexSort': indexSort , 'stype':'gp'}
    infos.append(info)
    ...
    for m in matchs:
        ...
        for key, value in asinVariationValues.items():
            ...
            future = executor.submit(
                getInfosThread, codeASIN, size_name, infos, indexSort , stype)
            futures.append(future)
    wait(futures)
    if infos == []:
        ...
        if priceElement != None:
            ...
            info = {'codeasin': codeASIN, 'sizeName': None,
                    'isSoldOut': isSoldOut, 'price': price , 'stype':'gp'}
            infos.append(info)
    print("END")
    return infos
...
def getPrice(codeAsin):
    link = link ='https://www.amazon.com/gp/product/{}/'.format(codeAsin)
    codeHTML = CodeHTML(link)
    root = codeHTML.tree()
    elements = root.xpath(
        '//*/td[@class="a-span12"]/span[@id="priceblock_ourprice"]')
